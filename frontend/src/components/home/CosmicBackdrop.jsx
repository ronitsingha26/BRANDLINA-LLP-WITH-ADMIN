...
function extendMaterial(BaseMaterial, cfg) {
  ...
  if (typeof defaults.roughness === "number" && uniforms.roughness) uniforms.roughness.value = defaults.roughness;
  if (typeof defaults.metalness === "number" && uniforms.metalness) uniforms.metalness.value = defaults.metalness;
  if (defaults.envMap && uniforms.envMap) uniforms.envMap.value = defaults.envMap;
  if (typeof defaults.envMapIntensity === "number" && uniforms.envMapIntensity) {
    uniforms.envMapIntensity.value = defaults.envMapIntensity;
  }
  defaults.dispose();

  Object.entries(cfg.uniforms ?? {}).forEach(([key, entry]) => {
    uniforms[key] = entry !== null && typeof entry === "object" && "value" in entry
      ? entry
      : { value: entry };
  });
...
function DirLight({ position, color }) {
  ...
  return <directionalLight ref={lightRef} color={color} intensity={1} position={position} />;
}
...
export function CosmicBackdrop({ config = {} }) {
  const settings = {
    beamWidth: 2,
    beamHeight: 15,
    beamNumber: 12,
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-0" aria-hidden="true">
      <BeamsScene
        ...
        beamNumber={settings.beamNumber}
